...
public class ChatRest {
	...
	public List<ChatMessage> getLatestChatMessages(@PathVariable String username) {
		...
		List<ChatMessage> messages = chatMessageRepos.findByUserSenderAndAppOrderByIdDesc(user, application);
	...
	public List<ChatMessage> getLatestChatMessages(@PathVariable String username, @PathVariable String receiver) {
		...
		List<ChatMessage> messages = chatMessageRepos.findByUserSenderAndUserReceiverAndAppOrderByIdDesc(user,
				receiver1, application);
	...
	public void putSeen(@PathVariable String selecteduser, @PathVariable String concteduser) {
		...
		User user = userService.findByUsername(selecteduser);
		List<ChatMessage> list = chatMessageRepos.findByUserReceiverUsernameAndUserSenderUsernameAndSeenAndApp(
				concteduser, selecteduser, false, application);
