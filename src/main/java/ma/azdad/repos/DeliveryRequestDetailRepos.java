...
public interface DeliveryRequestDetailRepos extends JpaRepository<DeliveryRequestDetail, Integer> {
	...
	@Query(select1 + " from DeliveryRequestDetail a where a.deliveryRequest.id != ?5 and a.deliveryRequest.project.id = ?1 and a.deliveryRequest.warehouse.id = ?2 and  a.deliveryRequest.type = ?3 and a.deliveryRequest.status in (?4) group by a.status, a.originNumber, a.partNumber.id, a.inboundDeliveryRequest.id")
	...
	@Query(select1 + " from DeliveryRequestDetail a where a.deliveryRequest.outboundDeliveryRequestReturn.id = ?1 and a.deliveryRequest.status in (?2) and a.deliveryRequest.id != ?3  group by a.status, a.originNumber, a.partNumber.id, a.inboundDeliveryRequest.id")
	...
	@Query(select4 + " from DeliveryRequestDetail a where a.deliveryRequest.outboundDeliveryRequestReturn.id = ?1 and a.deliveryRequest.status in (?2) group by a.partNumber.id")
	...
	@Query(select1 + " from DeliveryRequestDetail a where a.deliveryRequest.outboundDeliveryRequestTransfer.id = ?1 and a.deliveryRequest.status in (?2) and a.deliveryRequest.id != ?3  group by a.status, a.originNumber, a.partNumber.id, a.inboundDeliveryRequest.id")
	...
	@Query(select4 + " from DeliveryRequestDetail a where a.deliveryRequest.outboundDeliveryRequestTransfer.id = ?1 and a.deliveryRequest.status in (?2) group by a.partNumber.id")
	...
	@Query(select3 + "from DeliveryRequestDetail a where a.deliveryRequest.destinationProject.id = ?1 and a.deliveryRequest.type in (?2) and a.deliveryRequest.status in (?3) and a.quantity > " + usedQuantity3)
	...
	String select5 = "select new DeliveryRequestDetail(id,a.partNumber.name,a.partNumber.description,a.deliveryRequest.id,a.deliveryRequest.type,a.deliveryRequest.referenceNumber,unitCost,a.deliveryRequest.date4,a.deliveryRequest.project.name) ";
