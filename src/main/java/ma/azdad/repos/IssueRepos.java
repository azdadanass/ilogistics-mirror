...
public interface IssueRepos extends JpaRepository<Issue, Integer> {
	...
	String select1 = "select new Issue(a.id,a.status,a.severity,a.category,a.type,a.blocking,a.ownershipType" + //
	...
	@Query(select1 + "from Issue a where a.deliveryRequest.project.id = ?1")
	public List<Issue> findByProject(Integer projectId);

	@Query(select1 + "from Issue a where (a.deliveryRequest.requester.username = ?1 or a.deliveryRequest.project.manager.username = ?1 or a.deliveryRequest.project.id in (?2)) and a.status = ?3")
	...
	@Query(select1 + "from Issue a where a.ownershipUser.username = ?1 and a.status = ?2")
	...
	@Query(select1 + "from Issue a where a.user3.username = ?1 and a.status = ?2")
	...
	@Query("select distinct a.deliveryRequest.project.id from Issue a where a.deliveryRequest.project.id in (?1)")
	public List<Integer> findProjectIdList(Collection<Integer> userProjectIdList);
}
