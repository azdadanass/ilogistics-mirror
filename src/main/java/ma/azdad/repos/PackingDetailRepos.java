...
public interface PackingDetailRepos extends JpaRepository<PackingDetail, Integer> {
	...
	@Query("select count(*) from PackingDetail a where a.parent.partNumber.id = ?1 and a.hasSerialnumber is true")
	Long countByPartNumberAndHasSerialnumber(Integer partNumberId);
