...
//	String c10 = " select new StockRow(sum(a.quantity),a.inboundDeliveryRequest,a.partNumber.id,a.partNumber.name,a.partNumber.image,a.partNumber.description,a.partNumber.industryName,a.partNumber.categoryName,a.partNumber.typeName,a.partNumber.brandName,a.partNumber.internalPartNumberName,a.partNumber.internalPartNumberDescription) ";
	...
	String c24 = "select new StockRow(sum(a.quantity),a.status,a.deliveryRequestDetail.unitCost,a.deliveryRequestDetail.costCurrency.id,a.deliveryRequestDetail.unitPrice,a.deliveryRequestDetail.priceCurrency.id,a.deliveryRequest.project.name,a.deliveryRequest.warehouse.name,a.partNumber.id,a.partNumber.name,a.partNumber.image,a.partNumber.description,"
			+ brandName
			+ ",a.deliveryRequest.id,a.deliveryRequest.type,a.deliveryRequest.inboundType,a.deliveryRequest.reference,a.deliveryRequest.smsRef,a.deliveryRequest.date4,a.deliveryRequest.sdm,a.inboundDeliveryRequest.id,a.inboundDeliveryRequest.reference,"
			+ destinationProjectCustomerName + "," + destinationName + "," + originName + "," + destinationProjectName + ",a.deliveryRequest.deliverToCompanyType ," + deliverToCompanyName + " ,"
			+ deliverToCustomerName + "," + deliverToSupplierName + ",a.deliveryRequest.deliverToOther," + poNumero+","+inboundPoNumero + "," + endCustomerName + ") ";
...
//	String select17 = " select new StockRow(sum(a.quantity),a.status,a.deliveryRequest,a.inboundDeliveryRequest) ";
	...
	@Query(c10 + from1 + "  where  " + usernameCondition + " and (a.companyId = ?4 or a.inboundCompanyId = ?4) and datediff(current_date,a.inboundDeliveryRequest.date4) > a.inboundDeliveryRequest.approximativeStoragePeriod group by a.deliveryRequest.id,a.partNumber.id having sum(a.quantity) > 0")
	...
	@Query(c10 + from1 + "  where  " + usernameCondition + " and (a.customerId = ?4 or a.inboundCustomerId = ?4) and  datediff(current_date,a.inboundDeliveryRequest.date4) > a.inboundDeliveryRequest.approximativeStoragePeriod group by a.inboundDeliveryRequest.id,a.partNumber.id having sum(a.quantity) > 0")
...
//	public List<StockRow> findByCompanyOwnerGroupbyPartNumberAndDeliveryRequestOld(String username, List<Integer> warehouseList, List<Integer> assignedProjectList, Integer companyId);
	...
	@Query(c13 + "from StockRow a left join a.deliveryRequest.warehouse as warehouse left join a.deliveryRequest.destinationProject as destinationProject  where  " + usernameCondition + " and (a.companyId = ?4 or a.inboundCompanyId = ?4) and a.partNumber.stockItem = true group by a.partNumber.id,a.deliveryRequest.id")
