...
//				+ "</b> <br/><br/>";
		...
		row.addElement(new TD(deliveryRequest.getTransporter() != null ? deliveryRequest.getTransporter().getName() : "").setWidth(tdWidth2).setStyle(tdStyle2));
		table.addElement(row);
	...
	private void addMainContent(Document document, DeliveryRequest deliveryRequest) throws DocumentException, IOException {
		...
		addTableRow(table1, "Type", safeValue(getFullTypeValue(deliveryRequest)));
		...
		if ((DeliveryRequestType.INBOUND).equals(deliveryRequest.getType())) {
			addTableRow(table1, "Appr Storage Period",
					deliveryRequest.getApproximativeStoragePeriod() != null ? String.valueOf(deliveryRequest.getApproximativeStoragePeriod()) : "");
		}
		...
		if ((DeliveryRequestType.INBOUND).equals(deliveryRequest.getType())) {
			...
			addTableRow(table3, "Destination Site Address", safeValue(deliveryRequest.getDestination() != null ? deliveryRequest.getDestination().getGoogleAddress() : ""));
		}
...
//				String imageUrl = detail.getTypeImage(); // get packing detail type image from table PackingDetailTpe
	...
	private String getFullTypeValue(DeliveryRequest deliveryRequest) {
		...
		if ("Hardware Swap".equalsIgnoreCase(subType) && countByHardwareSwapInboundId(deliveryRequest.getId()) > 0) {
			if (result.length() > 0)
				result.append(", ");
	...
	public String generateStamp(PackingDetail packingDetail, DeliveryRequest deliveryRequest) {
		...
		try {
			...
			case COMPANY:
				image = deliveryRequest.getCompany().getLogo();
				break;
			case CUSTOMER:
				image = deliveryRequest.getCustomer().getPhoto();
				break;
			case SUPPLIER:
				image = deliveryRequest.getSupplier().getPhoto();
				break;
			default:
				break;
			}
			System.out.println("owner image path : "+UtilsFunctions.path() + image);
			logo = Image.getInstance(UtilsFunctions.path() + image);
...
//				logo = Image.getInstance(UtilsFunctions.path() + "resources/pdf/orange.png");
			...
			phrase.add(new Chunk("\nPacking Name : ", boldFont));
			phrase.add(new Chunk(packingDetail.getName(), normalFont));
...
//		map.forEach((pnId, unitPrice) -> stockRowRepos.updateUnitPriceByPartNumberAndOutboundDeliveryRequestReturn(unitPrice, pnId, outboundDeliveryRequestId));
	...
	public ma.azdad.mobile.model.DeliveryRequest findOneLightMobile(Integer id) {
		...
		if (dn.getIsInbound() && DeliveryRequestStatus.PARTIALLY_DELIVRED.equals(dn.getStatus())) {
			dn.getDetailList().forEach(i -> i.setTmpDeliveredQuantity(stockRowService.findQuantityByDeliveryRequestDetail(i.getId())));
		...
		if (dn.getRequester() != null) {
			dnm.setUser1(new ma.azdad.mobile.model.User(dn.getRequester().getUsername(), dn.getRequester().getFirstName(), dn.getRequester().getLastName(),
					dn.getRequester().getLogin(), dn.getRequester().getPhoto(), dn.getRequester().getEmail(), dn.getRequester().getJob()));
		...
		if (dn.getUser3() != null)
			dnm.setUser3(new ma.azdad.mobile.model.User(dn.getUser3().getUsername(), dn.getUser3().getFirstName(), dn.getUser3().getLastName(), dn.getUser3().getLogin(),
					dn.getUser3().getPhoto(), dn.getUser3().getEmail(), dn.getRequester().getJob()));
		if (dn.getDate3() != null)
		...
		if (dn.getUser4() != null)
			dnm.setUser4(new ma.azdad.mobile.model.User(dn.getUser4().getUsername(), dn.getUser4().getFirstName(), dn.getUser4().getLastName(), dn.getUser4().getLogin(),
					dn.getUser4().getPhoto(), dn.getUser4().getEmail(), dn.getRequester().getJob()));
		if (dn.getDate4() != null)
		...
		if (dn.getUser5() != null)
			dnm.setUser5(new ma.azdad.mobile.model.User(dn.getUser5().getUsername(), dn.getUser5().getFirstName(), dn.getUser5().getLastName(), dn.getUser5().getLogin(),
					dn.getUser5().getPhoto(), dn.getUser5().getEmail(), dn.getRequester().getJob()));
		if (dn.getDate5() != null)
		...
		if (dn.getUser8() != null)
			dnm.setUser8(new ma.azdad.mobile.model.User(dn.getUser8().getUsername(), dn.getUser8().getFirstName(), dn.getUser8().getLastName(), dn.getUser8().getLogin(),
					dn.getUser8().getPhoto(), dn.getUser8().getEmail(), dn.getRequester().getJob()));
		if (dn.getDate8() != null)
	...
	private void generateSerialNumberList(DeliveryRequest deliveryRequest) {
		...
		for (Integer stockRowId : deliveryRequest.getStockRowList().stream().map(i -> i.getId()).collect(Collectors.toList())) {
			...
			for (PackingDetail packingDetail : inboundStockRow.getPacking().getDetailList()) {
				...
				Integer maxPackingNumero = ObjectUtils
						.firstNonNull(deliveryRequestSerialNumberService.findMaxPackingNumero(inboundStockRow.getDeliveryRequestDetail().getId(), packingDetail.getId()), 0);
...
//					Double outboundQuantity = outboundDn.getDetailList().stream().filter(j -> j.getPartNumber().equals(i.getPartNumber())).mapToDouble(j -> j.getQuantity()).sum();
					...
					if (i.getQuantity().equals(outboundQuantity)) {
						...
						List<DeliveryRequestSerialNumber> outboundDrsnList = deliveryRequestSerialNumberService
								.findNotUsedByOutboundDeliveryRequestAndPartNumber(outboundDn.getId(), i.getPartNumberId(), usedSnList);
	...
	public void calculateMissingSerialNumberScript() {
		stockRowRepos.findDeliveryRequestListHavingPackingDetailRequiringSerialNumber().forEach(this::calculateMissingSerialNumber);
	}
	...
	public void updateNumberOfItems() {
		repos.updateNumberOfItems();
	}

	public void updateNumberOfItems(List<Integer> idList) {
		if (!idList.isEmpty())
			repos.updateNumberOfItems(idList);
	}

	public void updateNetWeight() {
		repos.updateNetWeight();
	}

	public void updateNetWeight(List<Integer> idList) {
		if (!idList.isEmpty())
			repos.updateNetWeight(idList);
	}

	public void updateGrossWeight() {
		repos.updateGrossWeight();
	}

	public void updateGrossWeight(List<Integer> idList) {
		if (!idList.isEmpty())
			repos.updateGrossWeight(idList);
	}
	...
	public void updateVolume(List<Integer> idList) {
		if (!idList.isEmpty())
			repos.updateVolume(idList);
	}

	public List<Integer> findAssociatedDeliveryRequestListWithPacking(Integer packingId) {
		return repos.findAssociatedDeliveryRequestListWithPacking(packingId);
	}
	...
	public List<ma.azdad.mobile.model.SerialNumberSummary> findSnSummary(Integer id) {
		...
		case INBOUND:
			...
			break;
		case OUTBOUND:
			list = deliveryRequest.getStockRowList().stream().filter(i -> i.getInboundDeliveryRequest().getIsSnRequired() && i.getPacking().getHasSerialnumber())
		...
		for (StockRow stockRow : list) {
			stockRow.getPacking().getDetailList().stream().filter(pd -> pd.getHasSerialnumber()).forEach(pd -> {
				...
				case OUTBOUND:
					usedQuantity = (double) list1.stream().filter(i -> i.getInboundStockRow().getDeliveryRequestDetail().getId().equals(inboundDeliveryRequestDetailId)
							&& i.getPackingDetail().getId().equals(pd.getId())).count();
					stockRowQuantity = -stockRow.getQuantity();
					break;
				}
				map.putIfAbsent(key, new SerialNumberSummary(inboundDeliveryRequestDetailId, pd.getId(), partNumberName, inboundDeliveryRequestId, inboundDeliveryRequestReference,
						packingName, pd.getType(), usedQuantity));
		...
		for (SerialNumberSummary sum : serialNumberSummaryList) {
			serialNumberSummaryListMobile.add(new ma.azdad.mobile.model.SerialNumberSummary(sum.getPackingName(), null, sum.getInboundDeliveryRequestReference(),
					sum.getInboundDeliveryRequestId(), sum.getPartNumberName(), sum.getPartNumberImage(), sum.getQuantity(), sum.getUsedQuantity()));
		}
		...
		for (DeliveryRequestSerialNumber sn : list) {
			...
			if (pnName != null && pnName.equalsIgnoreCase(partNumberName)) {
				mobileList.add(new ma.azdad.mobile.model.DeliveryRequestSerialNumber(sn.getId(), sn.getPackingDetail().getParent().getName(), pnName, sn.getSerialNumber(),
						sn.getPackingDetail().getParent().getPartNumber().getImage(), sn.getPackingDetail().getSnType(), sn.getPackingNumero(),
	...
	public List<ma.azdad.mobile.model.Issue> findIssueMobile(Integer id) {
		...
		for (Issue issue : list) {
			mbList.add(new ma.azdad.mobile.model.Issue(issue.getId(), issue.getSeverity(), issue.getStatus(), issue.getCategory().getName(), issue.getType().getName(),
					issue.getBlocking(), issue.getPermanent(), issue.getDescription(), issue.getDeliveryRequestReference(), issue.getProjectName(), "",
	...
	public ma.azdad.mobile.model.Issue findOneIssueMobile(Integer id) {
		...
		ma.azdad.mobile.model.Issue im = new ma.azdad.mobile.model.Issue(issue.getId(), issue.getSeverity(), issue.getStatus(), issue.getCategory().getName(),
				issue.getType().getName(), issue.getBlocking(), issue.getPermanent(), issue.getDescription(), issue.getDeliveryRequestReference(), issue.getProjectName(), "",
				issue.getUser1().getPhoto());
	...
	public List<String> findIssueByProjectAndParenType(Integer id) {
		...
		return categories.stream().map(IssueCategory::getName).collect(Collectors.toList());
	}

	public List<String> findIssueByCategory(Integer id, String name) {
		DeliveryRequest jr = findOne(id);
		IssueCategory issueCat = issueCategoryService.findByProjectAndParenTypeAndName(jr.getProjectId(), IssueParentType.DN, name).get(0);

		List<IssueType> issueTypes = issueTypeService.findByCategory(issueCat.getId());

		return issueTypes.stream().map(IssueType::getName).collect(Collectors.toList());
	}
	...
	public List<ma.azdad.mobile.model.User> findOwnerShipUserSelectionList(String type, Integer id, Integer supId) {
		...
		for (User user : result) {
			mbList.add(
					new ma.azdad.mobile.model.User(user.getUsername(), user.getFirstName(), user.getLastName(), user.getLogin(), user.getPhoto(), user.getEmail(), user.getJob()));
		}
	...
	public List<ma.azdad.mobile.model.User> addToNotifyUserMobile(Integer id) {
		 ...
		 * delegationService.findDelegateUserListByProject(dn.getProjectId()).forEach(i
		 * -> addToNotify(userService.findOneLight(i.getUsername()),users));
		 * //projectAssignmentService.findCompanyUserListAssignedToProject(issue.
		 * getProjectId()).forEach(i ->
		 * addToNotify(userService.findOneLight(i.getUsername())));
		 * projectAssignmentService.findUserListAssignedToProject(dn.getProjectId()).
		 * forEach(i -> addToNotify(userService.findOneLight(i.getUsername()),users));
		 * dn.getWarehouse().getManagerList().forEach(i->addToNotify(userService.
		 * findOneLight(i.getUser().getUsername()),users));
		 */
		/////
		addToNotify(dn.getRequester(), users);
		addToNotify(dn.getProject().getManager(), users);
		dn.getProject().getManagerList().stream().filter(i -> Arrays.asList(ProjectManagerType.HARDWARE_MANAGER, ProjectManagerType.QUALITY_MANAGER).contains(i.getType()))
				.forEach(i -> addToNotify(i.getUser(), users));
	...
	public void saveIssue(ma.azdad.mobile.model.Issue iss, ma.azdad.mobile.model.User user) {
		...
		IssueCategory issueCat = issueCategoryService.findByProjectAndParenTypeAndName(jr.getProjectId(), IssueParentType.DN, iss.getCategory()).get(0);
		...
		for (ToNotify toNotify : toNotifyList) {
			issue.getToNotifyList().add(new ma.azdad.model.ToNotify(userService.findByUsername(toNotify.getInternalResource().getUsername()), issue, toNotify.getNotifyByEmail(),
					toNotify.getNotifyBySms()));
		}
